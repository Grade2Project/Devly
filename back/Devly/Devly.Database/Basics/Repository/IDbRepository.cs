...
internal interface IDbRepository<TContext> where TContext : EfDbContext
{
    ...
    Task InsertAllAsync<TEntity>(IEnumerable<TEntity> entities, CancellationToken token = default)
        where TEntity : class;

    Task DeleteAsync<TEntity>(TEntity entity, CancellationToken token = default) where TEntity : class;

    Task DeleteAllAsync<TEntity>(IEnumerable<TEntity> entities, CancellationToken token = default)
        where TEntity : class;
